...
for idx_horizon = 1:num.horizon
    ...
    [v(idx_horizon), idx_P_slack, P_min] = solve_comm_sim(M,num,elems,params,snum,selems,sparams_step,Gconv,w);
    if ~v(idx_horizon).isconverge % if it did not converge
        ...
        delta_weighted(~cellfun(@(y)all(arrayfun(@(x) v(idx_horizon).rslts{y(x),x}.valid,1:num.sg)),v(idx_horizon).delta(2,2:end))) = nan;
        if all(isnan(delta_weighted))
            idx_min = v(idx_horizon).iter;
        else
            [~,idx_min] = min(delta_weighted);
            idx_min = v(idx_horizon).delta{2,idx_min+1};
        end
