function [v, idx_Pslack, P_min] = solve_comm(M,num,elems,params,sn,se,sparams_step, Gconv,w)
%SOVLE_COMM  Coordinates the iterative solving of a step of the dmpc problem.
%
%   [v, idx_P_slack, P_min] = SOLVE_COMM(M,num,elems,params,sn,se,sparams_step, Gconv,w)
%
%   DESCRIPTION:
...
%   iterations to max in w. Will break if obviously diverging. Will 
%   activate restoration prodeduring if converging but infeasible. 
%
%   INPUTS:
...
%
%   DEPENDENCIES: check_convergence, create_pass_var, solve_unconverged,
%   update_ig
...
while any(~isconverge) && max(cntr)<n_iter_max
    ...
    rslts_new = solve_unconverged(isconverge,sparams_step,Ma, se, idx_Pslack);
    idx_loop = 0;
    for idx_sg = find(~isconverge)
        idx_loop = idx_loop+1;
        rslts{cntr(idx_sg),idx_sg} = rslts_new{1,idx_loop};
    end
    % Break if obviously diverged
    if max(cellfun(@(ri)abs(ri.cost),rslts_new),[],'all')>10^5
        isconverge = false(1,num.sg);
        break
    end
